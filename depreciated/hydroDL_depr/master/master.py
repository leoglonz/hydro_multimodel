...
def loadModel(out, epoch=None):
    ...
    model = hydroDL_depr.model.train.loadModel(out, epoch)
    return model
...
def  loadData(optData, readX=True, readY=True):
    if eval(optData['name']) is hydroDL_depr.data.dbCsv.DataframeCsv:
        df = hydroDL_depr.data.dbCsv.DataframeCsv(
        ...
        if readX is True:
            ...
            x = None
            c = None
    elif eval(optData['name']) is hydroDL_depr.data.camels.DataframeCamels:
        df = hydroDL_depr.data.camels.DataframeCamels(
    ...
    elif eval(optData['name']) is hydroDL_depr.data.gages.DataframeGages:
        df = hydroDL_depr.data.gages.DataframeGages(
            subset=optData['subset'], tRange=optData['tRange'])
        x = df.getDataTs(
            varLst=optData['varT'],
            doNorm=optData['doNorm'][0],
...
def train(mDict):
    ...
    # loss
    if eval(optLoss['name']) is hydroDL_depr.model.crit.SigmaLoss:
        lossFun = hydroDL_depr.model.crit.SigmaLoss(prior=optLoss['prior'])
        optModel['ny'] = ny * 2
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.RmseLoss:
        lossFun = hydroDL_depr.model.crit.RmseLoss()
        optModel['ny'] = ny
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.NSELoss:
        lossFun = hydroDL_depr.model.crit.NSELoss()
        optModel['ny'] = ny
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.NSELosstest:
        lossFun = hydroDL_depr.model.crit.NSELosstest()
        optModel['ny'] = ny
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.MSELoss:
        lossFun = hydroDL_depr.model.crit.MSELoss()
        optModel['ny'] = ny
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.RmseLossCNN:
        lossFun = hydroDL_depr.model.crit.RmseLossCNN()
        optModel['ny'] = ny
    elif eval(optLoss['name']) is hydroDL_depr.model.crit.ModifyTrend1:
        lossFun = hydroDL_depr.model.crit.ModifyTrend1()
        optModel['ny'] = ny
    ...
    if optModel['nx'] != nx:
        ...
        optModel['nx'] = nx
    if eval(optModel['name']) is hydroDL_depr.model.rnn.CudnnLstmModel:
        if type(x) is tuple:
        ...
        model = hydroDL_depr.model.rnn.CudnnLstmModel(
            nx=optModel['nx'],
            ny=optModel['ny'],
            hiddenSize=optModel['hiddenSize'])
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CpuLstmModel:
        model = hydroDL_depr.model.rnn.CpuLstmModel(
            nx=optModel['nx'],
            ny=optModel['ny'],
            hiddenSize=optModel['hiddenSize'])
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.LstmCloseModel:
        model = hydroDL_depr.model.rnn.LstmCloseModel(
            nx=optModel['nx'],
            ny=optModel['ny'],
            hiddenSize=optModel['hiddenSize'],
            fillObs=True)
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.AnnModel:
        model = hydroDL_depr.model.rnn.AnnCloseModel(
            nx=optModel['nx'],
            ny=optModel['ny'],
            hiddenSize=optModel['hiddenSize'])
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.AnnCloseModel:
        model = hydroDL_depr.model.rnn.AnnCloseModel(
            nx=optModel['nx'],
            ny=optModel['ny'],
            hiddenSize=optModel['hiddenSize'],
            fillObs=True)
    elif eval(optModel['name']) is hydroDL_depr.model.cnn.LstmCnn1d:
        convpara = optModel['convNKSP']
        model = hydroDL_depr.model.cnn.LstmCnn1d(
            nx=optModel['nx'],
            ny=optModel['ny'],
            rho = optModel['rho'],
            nkernel=convpara[0],
            ...
            stride=convpara[2],
            padding = convpara[3])
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CNN1dLSTMmodel:
        ...
        if type(daobsOption) is list:
            if len(daobsOption)-3 >= 7:
                ...
                model = hydroDL_depr.model.rnn.CNN1dLSTMmodel(
                        nx=optModel['nx'],
                        ny=optModel['ny'],
    ...
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CNN1dLSTMInmodel:
        # daobsOption = optData['daObs']
        ...
        if type(daobsOption) is list:
            if len(daobsOption)-3 >= 7:
                ...
                optModel['nobs'] = len(daobsOption)
                convpara = optModel['convNKS']
                model = hydroDL_depr.model.rnn.CNN1dLSTMInmodel(
                        nx=optModel['nx'],
                        ny=optModel['ny'],
                        # nobs=optModel['nobs']-3,
            ...
            raise Exception('CNN kernel used but daobs option is not obs list')
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CNN1dLCmodel:
        ...
        if len(x[1].shape)==2:
            ...
            optModel['nobs'] = x[1].shape[1]
        convpara = optModel['convNKS']
        model = hydroDL_depr.model.rnn.CNN1dLCmodel(
                nx=optModel['nx'],
                ny=optModel['ny'],
                nobs=optModel['nobs'],
                hiddenSize=optModel['hiddenSize'],
    ...
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CNN1dLCInmodel:
        LCrange = optData['lckernel']
        tLCLst = utils.time.tRange2Array(LCrange)
        optModel['nobs'] = x[1].shape[-1]
        convpara = optModel['convNKS']
        model = hydroDL_depr.model.rnn.CNN1dLCInmodel(
                nx=optModel['nx'],
                ny=optModel['ny'],
                nobs=optModel['nobs'],
                hiddenSize=optModel['hiddenSize'],
                ...
                poolOpt=optModel['poolOpt'])
        print('CNN1d Local calibartion Kernel is used!')
    elif eval(optModel['name']) is hydroDL_depr.model.rnn.CudnnInvLstmModel:
        # optModel['ninv'] = x[1].shape[-1]
        optModel['ninv'] = x[1].shape[-1]+c.shape[-1] # Test the inv using attributes
        model = hydroDL_depr.model.rnn.CudnnInvLstmModel(
                nx=optModel['nx'],
                ny=optModel['ny'],
                hiddenSize=optModel['hiddenSize'],
                ninv = optModel['ninv'],
    ...
    model = hydroDL_depr.model.train.trainModel(
        model,
        x,
        y,
        c,
...
def test(out,
    ...
    if reTest is True:
        ...
        if closedLoop is not None:
            ...
            for iloop in range(0, nLoop):
                ...
                hydroDL_depr.model.train.testModel(
    ...
    if optData['doNorm'][1] is True:
        if eval(optData['name']) is hydroDL_depr.data.dbCsv.DataframeCsv:
            ...
            nTar = len(target)
            for k in range(nTar):
                pred[:, :, k] = hydroDL_depr.data.dbCsv.transNorm(
                    pred[:, :, k],
                    rootDB=optData['rootDB'],
                    fieldName=target[k],
                    fromRaw=False)
                obs[:, :, k] = hydroDL_depr.data.dbCsv.transNorm(
                    obs[:, :, k],
                    rootDB=optData['rootDB'],
                    fieldName=target[k],
                    fromRaw=False)
                if isSigmaX is True:
                    sigmaX[:, :, k] = hydroDL_depr.data.dbCsv.transNormSigma(
                        sigmaX[:, :, k],
                        rootDB=optData['rootDB'],
                        fieldName=target[k],
                        fromRaw=False)
            ...
            pred = hydroDL_depr.data.camels.transNorm(
                pred, targetstr, toNorm=False)
            obs = hydroDL_depr.data.camels.transNorm(obs, 'usgsFlow', toNorm=False)
            if basinnorm is True:
                if type(subset) is list:
                    ...
                    gageid = subset
                for ii in range(nvar):
                    pred[:,:,ii] = hydroDL_depr.data.camels.basinNorm(
                        pred[:,:,ii], gageid=gageid, toNorm=False)
                obs = hydroDL_depr.data.camels.basinNorm(
