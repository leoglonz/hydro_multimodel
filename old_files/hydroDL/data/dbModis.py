...
dateStrLst = [
    x
    for x in os.listdir(modisDir)
    if dt.strptime(x, "%Y.%m.%d") >= tRg[0] and dt.strptime(x, "%Y.%m.%d") <= tRg[1]
]
...
conn = psycopg2.connect(
    database="gisdb", user="postgres", host="localhost", password="19900323", port=5432
)
cursor = conn.cursor()
bTable = False
try:
    cursor = conn.cursor()
    cursor.execute(r"SELECT EXISTS(SELECT fpar FROM modis)")
    bTable = True
