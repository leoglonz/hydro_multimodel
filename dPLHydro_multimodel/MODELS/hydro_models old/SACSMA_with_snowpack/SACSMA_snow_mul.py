...
def has_nans_or_negatives(tensor):
    has_nans = torch.isnan(tensor).any()
    has_negatives = (tensor < 0).any()
    return has_nans, has_negatives
...
class SACSMA_snow_Mul(torch.nn.Module):
    ...
    def forward(self, x_hydro_model, c_hydro_model, params_raw, args, muwts=None, warm_up=0, init=False, routing=False, comprout=False, conv_params_hydro=None):
        ...
        if warm_up > 0:
            ...
            SNOWPACK = (torch.zeros([Ngrid, nmul], dtype=torch.float32) + 0.0001).to(args["device"])
            MELTWATER = (torch.zeros([Ngrid, nmul], dtype=torch.float32) + 0.0001).to(args["device"])
        ...
        for t in range(Nstep):
            ...
            LZTW_storage_orig = LZTW_storage
            ...
            # print(f'log: LZTW_storage = {LZTW_storage_orig} \n LZTW_w_twexl = {LZTW_storage_exl} \n LZTW_w_Rls = {LZTW_storage}')
            has_nans1, has_negatives1 = has_nans_or_negatives(LZTW_storage_orig)
            has_nans2, has_negatives2 = has_nans_or_negatives(flux_twexl)
            # has_nans2, has_negatives2 = has_nans_or_negatives(LZTW_storage_exl)
            has_nans3, has_negatives3 = has_nans_or_negatives(LZTW_storage)
            

            if has_nans1 > 0 or has_negatives1 > 0:
                print(f'Tensor 1 has NaNs: {has_nans1}, has negatives: {has_negatives1}')
                print(f'log: LZTW_storage = {flux_twexl}') # \n LZTW_w_twexl = {LZTW_storage_exl} \n LZTW_w_Rls = {LZTW_storage}')
                exit()
            
            if has_nans2 > 0 or has_negatives2 > 0:
                print(f'Tensor 2 has NaNs: {has_nans2}, has negatives: {has_negatives2}')
                print(f'log: LZTW_storage = {flux_twexl}') # \n LZTW_w_twexl = {LZTW_storage_exl} \n LZTW_w_Rls = {LZTW_storage}')
                exit()
            ...
            # print(f'log2: flux_Pcfwp = {flux_Pcfwp} \n flux_twexlp = {flux_twexlp} \n LZFWP_storage = {LZFWP_storage}')
            # has_nans1, has_negatives1 = has_nans_or_negatives(flux_Pcfwp)
            # has_nans2, has_negatives2 = has_nans_or_negatives(flux_twexlp)
            # has_nans3, has_negatives3 = has_nans_or_negatives(LZFWP_storage)
            
            if has_nans1 > 0 or has_negatives1 > 0:
                print(f'Tensor 1 has NaNs: {has_nans1}, has negatives: {has_negatives1}')
            if has_nans2 > 0 or has_negatives2 > 0:
                print(f'Tensor 2 has NaNs: {has_nans2}, has negatives: {has_negatives2}')
            if has_nans3 > 0 or has_negatives3 > 0:
                print(f'Tensor 3 has NaNs: {has_nans3}, has negatives: {has_negatives3}')
                print(f'log: LZTW_storage = {LZTW_storage_orig} \n LZTW_w_twexl = {LZTW_storage_exl} \n LZTW_w_Rls = {LZTW_storage}')
        ...
        if init:  # means we are in warm up. here we just return the storages to be used as initial values
            return Qsrout, SNOWPACK, MELTWATER, UZTW_storage, UZFW_storage, LZTW_storage, \
