...
# class AnnModel_bn(torch.nn.Module):
#     def __init__(self, *, nx, ny, hiddenSize, dropout_rate=0.5):
#         super(AnnModel_bn, self).__init__()
#         self.hiddenSize = hiddenSize
#         self.i2h = nn.Linear(nx, hiddenSize)
#         self.h2h1 = nn.Linear(hiddenSize, hiddenSize, bias=True)
#         self.h2h2 = nn.Linear(hiddenSize, hiddenSize, bias=True)
...
#         self.bn3 = nn.BatchNorm1d(hiddenSize)
#         self.bn4 = nn.BatchNorm1d(hiddenSize)
#         self.bn5 = nn.BatchNorm1d(hiddenSize)
#         self.bn6 = nn.BatchNorm1d(hiddenSize)
#         self.bn7 = nn.BatchNorm1d(hiddenSize)
#         self.reset_parameters()

#     def reset_parameters(self):
#         std = 1.0 / math.sqrt(self.hiddenSize)
#         for layer in [self.i2h, self.h2h1, self.h2h2, self.h2h3, self.h2h4, self.h2h5, self.h2h6, self.h2o]:
#             layer.weight.data.uniform_(-std, std)
#             if layer.bias is not None:
#                 layer.bias.data.uniform_(-std, std)



#     def forward(self, x, y=None):
#        # assert not torch.isnan(x).any(), "NaNs found in input"
        
#         ht = self.bn1(F.relu(self.i2h(x)))
#         ht = self.dropout(ht)
#        # assert not torch.isnan(ht).any(), "NaNs found after i2h"

#         ht1 = self.bn2(F.relu(self.h2h1(ht)))
#         ht1 = self.dropout(ht1)
#        # assert not torch.isnan(ht1).any(), "NaNs found after h2h1"

#         ht2 = self.bn3(F.relu(self.h2h2(ht1)))
#         ht2 = self.dropout(ht2)
#        # assert not torch.isnan(ht2).any(), "NaNs found after h2h2"

#         ht3 = self.bn4(F.relu(self.h2h3(ht2)))
#         ht3 = self.dropout(ht3)
#        # assert not torch.isnan(ht3).any(), "NaNs found after h2h3"

#         ht4 = self.bn5(F.relu(self.h2h4(ht3)))
#         ht4 = self.dropout(ht4)
#        # assert not torch.isnan(ht4).any(), "NaNs found after h2h4"

#         ht5 = self.bn6(F.relu(self.h2h5(ht4)))
#         ht5 = self.dropout(ht5)
#        # assert not torch.isnan(ht5).any(), "NaNs found after h2h5"

#         ht6 = self.bn7(F.relu(self.h2h6(ht5)))
#         ht6 = self.dropout(ht6)
#        # assert not torch.isnan(ht6).any(), "NaNs found after h2h6"

#         yt = torch.sigmoid(self.h2o(ht6))
#        # assert not torch.isnan(yt).any(), "NaNs found in output"
#         return yt
...
class HBVMulTDET(torch.nn.Module):
    ...
    def forward(self, x, parameters, staind, tdlst, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False,
        ...
        Qsimmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(parhbvFull)
        ETmu = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(parhbvFull)

        # Output the box components of Q
        Qsimmu0 = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(parhbvFull)
        Qsimmu1 = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(parhbvFull)
        Qsimmu2 = (torch.zeros(Pm.size(), dtype=torch.float32) + 0.001).to(parhbvFull)
...
class HBVMulTDET_DA(torch.nn.Module):
    """HBV Model with multiple components and dynamic parameters PyTorch version"""
    # Add an ET shape parameter for the original ET equation; others are the same as HBVMulTD()
    # we suggest you read the class HBVMul() with original static parameters first
    ...
    def __init__(self):
        """Initiate a HBV instance"""
        super(HBVMulTDET_DA, self).__init__()

    def forward(self, x, parameters, initial_S,staind, tdlst, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False,
                comprout=False, dydrop=False):
        # Modified from the original numpy version from Beck et al., 2020. (http://www.gloh2o.org/hbv/) which
        # runs the HBV-light hydrological model (Seibert, 2005).
        # NaN values have to be removed from the inputs.
...
class MultiInv_HBVTDModel(torch.nn.Module):
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        return out #,hbvpara,routpara


class MultiInv_SACSMAModel(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False):
        # LSTM Inv + HBV Forward
        super(MultiInv_SACSMAModel, self).__init__()
        self.ninv = ninv
        self.nfea = nfea
        self.hiddeninv = hiddeninv
        self.nmul = nmul
        ...
        self.SACSMA = HydroModels.SACSMA()
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        if self.nwtspm == 0: # simple average for multiple components
            ...
            wtspara = Params0[-1, :, -self.nwtspm:]
            wts = F.softmax(wtspara, dim=-1)  # softmax to make sure sum to 1
        out = self.SACSMA(x, parameters=hbvpara, staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        return out


class MultiInv_HBVTDModel_water_loss(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters

    # Gage_level HBV and transfer flow with LSTM
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False,model_name = 'waterlossv1'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"

        try:
            PBMmodel = getattr(importlib.import_module(model_import_string), 'HBVMulTDET_water_loss')
        except ImportError:
    ...
    def forward(self, x, z, Ai_batch, Ac_batch, idx_matric,  doDropMC=False):
        Params0 = self.lstminv(z) # dim: Time, Gage, Para
        ntstep = Params0.shape[0]
        ngage = Params0.shape[1]
        # print(Params0)
        ...
        out = self.HBV(x, parameters=hbvpara,Ai_batch = Ai_batch, Ac_batch = Ac_batch, idx_matric = idx_matric,  staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        # return hbvpara,routpara
...
class MultiInv_HBVTDModel_water_loss_v2(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Gage_level HBV and MERIT_lelvel transfer flow with LSTMs
    def __init__(self, *, ninv1, nfea1, nmul1, hiddeninv1,ninv2, nfea2,hiddeninv2, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v2, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
        ...
        if compwts is False:
            ...
            nwtspm = nmul1
        ntp_1 = nhbvpm + nroutpm + nwtspm 
        
        ntp_2 = nfea2
        # ntp = nfea*nmul+nmul+2
        # ntp = nfea * nmul + 2
        self.lstminv1 = CudnnLstmModel(
            nx=ninv1, ny=ntp_1, hiddenSize=hiddeninv1, dr=drinv)
        
        self.lstminv2 = CudnnLstmModel(
            nx=ninv2, ny=ntp_2, hiddenSize=hiddeninv2, dr=drinv)
    ...
    def forward(self, x, z1,z2, Ai_batch, Ac_batch, idx_matric,  doDropMC=False):
        Params0 = self.lstminv1(z1) # dim: Time, Gage, Para
        ntstep = Params0.shape[0]
        ngage = Params0.shape[1]
        # print(Params0)
        hbvpara0 = Params0[:, :, 0:self.nhbvpm]
        # hbvpara = torch.clamp(hbvpara0, min=0.0, max=1.0).view(ngage, self.nfea, self.nmul)
        hbvpara = torch.sigmoid(hbvpara0).view(ntstep, ngage, self.nfea1, self.nmul1) # hbv scaled para, [0,1]
        routpara0 = Params0[-1, :, self.nhbvpm:self.nhbvpm+self.nroutpm] # routing para dim:[Ngage, nmul*2] or [Ngage, 2]
        ...
        if self.comprout is False:
            ...
            # routpara = torch.clamp(routpara0, min=0.0, max=1.0).view(ngage*self.nmul, 2) # first dim:gage*component
            routpara = torch.sigmoid(routpara0).view(ngage * self.nmul1, 2) # [0,1]
        if self.nwtspm == 0: # simple average for multiple components
            wts = None
        else: # weighted average using learned weights
            wtspara = Params0[-1, :, -self.nwtspm:]
        ...
        waterloss_parameters = self.lstminv2(z2) # dim: Time, Gage, Para
        waterloss_parameters = torch.sigmoid(waterloss_parameters[-1, :,:])

        out = self.HBV(x, parameters=hbvpara,waterloss_parameters = waterloss_parameters, Ai_batch = Ai_batch, Ac_batch = Ac_batch, idx_matric = idx_matric,  staind=self.staind, tdlst=self.tdlst, mu=self.nmul1, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        # return hbvpara,routpara
...
class MultiInv_HBVTDModel_water_loss_v3(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # MERIT_lelvel HBV and MERIT_lelvel transfer flow with LSTM
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False,model_name = 'waterlossv1'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v3, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
...
class MultiInv_HBVTDModel_water_loss_v4(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Gage_level HBV with LSTM and MERIT_lelvel transfer flow with ANN
    def __init__(self, *, ninv1, nfea1, nmul1, hiddeninv1,ninv2, nfea2,nmul2,hiddeninv2, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v4, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
        ...
        self.Ann = AnnModel(
            nx=ninv2, ny=ntp_2, hiddenSize=hiddeninv2, dropout_rate=drinv)
    ...
    def forward(self, x, z1,z2, Ai_batch, Ac_batch, idx_matric,  doDropMC=False):
        ...
        nmerit = z2.shape[0]
        waterloss_parameters = self.Ann(z2).view(nmerit, self.nfea2, self.nmul2) # dim: Time, Gage, Para
...
class MultiInv_HBVTDModel_water_loss_v5(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Gage_level HBV with LSTM and MERIT_lelvel transfer flow with ANN
    def __init__(self, *, ninv1, nfea1, nmul1, hiddeninv1,ninv2, nfea2,nmul2,hiddeninv2,  drinv1=0.5,drinv2=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v5, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
        ...
        ntp_2 = nfea2*nmul2 + nroutpm
        # ntp = nfea*nmul+nmul+2
        # ntp = nfea * nmul + 2
        self.lstminv1 = CudnnLstmModel(
            nx=ninv1, ny=ntp_1, hiddenSize=hiddeninv1, dr=drinv1)
        
        self.Ann = AnnModel(
            nx=ninv2, ny=ntp_2, hiddenSize=hiddeninv2, dropout_rate=drinv2)
    ...
    def forward(self, x, z1,z2, Ai_batch, Ac_batch, idx_matric,  doDropMC=False):
        ...
        #hbvpara0 = Params0[:, :, 0:self.nhbvpm]
        # hbvpara = torch.clamp(hbvpara0, min=0.0, max=1.0).view(ngage, self.nfea, self.nmul)
        hbvpara = torch.sigmoid(Params0).view(ntstep, ngage, self.nfea1, self.nmul1) # hbv scaled para, [0,1]
        

        nmerit = z2.shape[0]
        waterloss_parameters0 = self.Ann(z2)
        waterloss_parameters = waterloss_parameters0[:,:self.nfea2*self.nmul2].view(nmerit, self.nfea2, self.nmul2) # dim: Time, Gage, Para
        
        routpara = waterloss_parameters0[:, self.nfea2*self.nmul2:self.nfea2*self.nmul2+self.nroutpm] # routing para dim:[Ngage, nmul*2] or [Ngage, 2]
        wts = None
        out = self.HBV(x, parameters=hbvpara,waterloss_parameters = waterloss_parameters, Ai_batch = Ai_batch, Ac_batch = Ac_batch, idx_matric = idx_matric,  staind=self.staind, tdlst=self.tdlst, mu=self.nmul1, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, comprout=self.comprout, dydrop=self.dydrop)
        # return hbvpara,routpara
...
class LstmModel_torch(torch.nn.Module):
    def __init__(self, *, nx, ny, hiddenSize, dr=0.5):
...
class MultiInv_HBVTDModel_water_loss_v6(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Gage_level HBV with LSTM and MERIT_lelvel transfer flow with ANN
    def __init__(self, *, ninv1, nfea1, nmul1, hiddeninv1,ninv2, nfea2,nmul2,hiddeninv2,  drinv1=0.5,drinv2=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v6, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
...
class MultiInv_HBVTDModel_water_loss_v7(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Merit_level HBV with LSTM 
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v7, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
...
class MultiInv_HBVTDModel_water_loss_v8(torch.nn.Module):
    # class for dPL + HBV with multiple components and some dynamic parameters
    # Gage_level HBV with LSTM and MERIT_lelvel transfer flow with ANN
    def __init__(self, *, ninv1, nfea1, nmul1, hiddeninv1,ninv2, nfea2,nmul2,hiddeninv2, hiddeninv_rout, drinv1=0.5,drinv2=0.5, inittime=0, routOpt=False, comprout=False,
                 compwts=False, staind=-1, tdlst=[], dydrop=0.0, ETMod=False, model_name = 'waterlossv4'):
        # LSTM Inv + HBV Forward
        super(MultiInv_HBVTDModel_water_loss_v8, self).__init__()
        package_name = "hydroDL.model.HydroModels"
        model_import_string = f"{package_name}.{model_name}"
        ...
        self.Ann_rout = AnnModel(
            nx=ninv2, ny=nroutpm, hiddenSize=hiddeninv_rout, dropout_rate=drinv2)
    ...
    def forward(self, x, z1,z2, z_rout, Ai_batch, Ac_batch, idx_matric,  doDropMC=False):
        Params0 = self.lstminv1(z1) # dim: Time, Gage, Para
        ntstep = Params0.shape[0]
        ngage = Params0.shape[1]
        # print(Params0)
        ...
        ngage = z_rout.shape[0]
        routpara = self.Ann_rout(z_rout)
