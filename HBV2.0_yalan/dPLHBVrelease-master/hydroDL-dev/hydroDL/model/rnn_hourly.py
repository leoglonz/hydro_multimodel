...
class HBVMulET_hourly(torch.nn.Module):
    """Multi-component HBV Model PyTorch version"""
    # Add an ET shape parameter; others are the same as class HBVMul()
    # refer HBVMul() for detailed comments
    ...
    def __init__(self):
        """Initiate a HBV instance"""
        super(HBVMulET_hourly, self).__init__()
    ...
    def forward(self, x, parameters, mu, muwts, rtwts, bufftime=0, outstate=False, routOpt=False, comprout=False):
        ...
        for t in range(Nstep):
            ...
            hourstep = 1
            timestep = torch.tensor(hourstep / 24).to(PRECIP)
            for step in range(int(24 / hourstep)):

                RAIN = torch.mul(PRECIP, (TEMP >= parTT).type(torch.float32))*timestep
                SNOW = torch.mul(PRECIP, (TEMP< parTT).type(torch.float32))*timestep

                # Snow
                SNOWPACK = SNOWPACK + SNOW
                melt = parCFMAX * (TEMP - parTT)*timestep
                # melt[melt < 0.0] = 0.0
                melt = torch.clamp(melt, min=0.0)
                # melt[melt > SNOWPACK] = SNOWPACK[melt > SNOWPACK]
                melt = torch.min(melt, SNOWPACK)
                MELTWATER = MELTWATER + melt
                SNOWPACK = SNOWPACK - melt
                refreezing = parCFR * parCFMAX * (parTT - TEMP)*timestep
                # refreezing[refreezing < 0.0] = 0.0
                # refreezing[refreezing > MELTWATER] = MELTWATER[refreezing > MELTWATER]
                refreezing = torch.clamp(refreezing, min=0.0)
                refreezing = torch.min(refreezing, MELTWATER)
                SNOWPACK = SNOWPACK + refreezing
                MELTWATER = MELTWATER - refreezing
                tosoil = (MELTWATER - (parCWH * SNOWPACK))*timestep
                # tosoil[tosoil < 0.0] = 0.0
                tosoil = torch.clamp(tosoil, min=0.0)
                MELTWATER = MELTWATER - tosoil
                ...
                # soil_wetness[soil_wetness > 1.0] = 1.0
                soil_wetness = torch.clamp(soil_wetness, min=0.0, max=1.0)
                recharge = (RAIN + tosoil) * soil_wetness*timestep


                SM = SM + RAIN + tosoil - recharge
                excess = (SM - parFC)*timestep
                # excess[excess < 0.0] = 0.0
                excess = torch.clamp(excess, min=0.0)
                SM = SM - excess
                # MODIFY HERE. Add the shape para parBETAET for ET equation
                ...
                # evapfactor[evapfactor > 1.0] = 1.0
                evapfactor  = torch.clamp(evapfactor, min=0.0, max=1.0)
                ETact = EVAP_P * evapfactor*timestep
                ETact = torch.min(SM, ETact)
                SM = torch.clamp(SM - ETact, min=PRECS) # SM can not be zero for gradient tracking

                # Groundwater boxes
                SUZ = SUZ + recharge + excess
                PERC = torch.min(SUZ, parPERC)*timestep
                SUZ = SUZ - PERC
                Q0 = parK0 * torch.clamp(SUZ - parUZL, min=0.0)*timestep
                SUZ = SUZ - Q0
                Q1 = parK1 * SUZ*timestep
                SUZ = SUZ - Q1
                SLZ = SLZ + PERC
                Q2 = parK2 * SLZ*timestep
                SLZ = SLZ - Q2
                Qsimmu[t, :, :] = Qsimmu[t, :, :] + Q0 + Q1 + Q2
...
class HBVMulTDET_RK(torch.nn.Module):
    ...
    def __init__(self,scheme = "continuous"):
        """Initiate a HBV instance"""
        super(HBVMulTDET_RK, self).__init__()
        self.scheme = scheme
    def forward(self, x, parameters, staind, tdlst, mu, muwts, rtwts,hourstep = 24, bufftime=0, outstate=False, routOpt=False,
                comprout=False, dydrop=False):
        # Modified from the original numpy version from Beck et al., 2020. (http://www.gloh2o.org/hbv/) which
        # runs the HBV-light hydrological model (Seibert, 2005).
        # NaN values have to be removed from the inputs.
        ...
        timestep = torch.tensor(hourstep / 24.0).to(x)
        ...
        parstaFull = parAllTrans[staind, :, :, :].unsqueeze(0).repeat([parAllTrans.size()[0], 1, 1, 1])  # static para matrix
        parhbvFull = torch.clone(parstaFull)
        ...
        for t in range(Nstep):
            ...
            if self.scheme == "discrete":
                RHS = self.RHS_discrete
            elif self.scheme == "continuous":
                RHS = self.RHS_continuous
            dSNOWPACK1,dMELTWATER1,dSM1,dSUZ1,dSLZ1 = RHS(PRECIP, TEMP, EVAP_P, SNOWPACK, MELTWATER, SM, SUZ, SLZ, parBETA, parFC, parK0, parK1, parK2,
                parLP, parPERC, parUZL, parTT, parCFMAX, parCFR, parCWH, parBETAET)
            if t<Nstep-1:
                PRECIP = (Pm[t, :, :]+Pm[t+1, :, :])/2
                TEMP = (Tm[t, :, :]+Tm[t+1, :, :])/2
                EVAP_P = (ETpm[t, :, :]+ETpm[t+1, :, :])/2

            SNOWPACK1 = SNOWPACK+dSNOWPACK1/(2.0); MELTWATER1 = MELTWATER+dMELTWATER1/(2.0);SM1 = SM+dSM1/(2.0);SUZ1 =SUZ+dSUZ1/(2.0);SLZ1 = SLZ+dSLZ1/(2.0);
            dSNOWPACK2, dMELTWATER2, dSM2, dSUZ2, dSLZ2 = RHS(PRECIP, TEMP, EVAP_P, SNOWPACK1, MELTWATER1, SM1, SUZ1,
                                                                   SLZ1, parBETA, parFC, parK0, parK1, parK2,
                                                                   parLP, parPERC, parUZL, parTT, parCFMAX, parCFR,
                                                                   parCWH, parBETAET)
            SNOWPACK2 = SNOWPACK + dSNOWPACK2 / (2.0);
            MELTWATER2 = MELTWATER + dMELTWATER2 / (2.0);
            SM2 = SM + dSM2 / (2.0);
            SUZ2 = SUZ + dSUZ2 / (2.0);
            SLZ2 = SLZ + dSLZ2 / (2.0);

            dSNOWPACK3, dMELTWATER3, dSM3, dSUZ3, dSLZ3 = RHS(PRECIP, TEMP, EVAP_P, SNOWPACK2, MELTWATER2, SM2, SUZ2,
                                                                   SLZ2, parBETA, parFC, parK0, parK1, parK2,
                                                                   parLP, parPERC, parUZL, parTT, parCFMAX, parCFR,
                                                                   parCWH, parBETAET)

            SNOWPACK3 = SNOWPACK + dSNOWPACK3 ;
            MELTWATER3 = MELTWATER + dMELTWATER3 ;
            SM3 = SM + dSM3 ;
            SUZ3 = SUZ + dSUZ3 ;
            SLZ3 = SLZ + dSLZ3 ;
            ...
            dSNOWPACK4, dMELTWATER4, dSM4, dSUZ4, dSLZ4 = RHS(PRECIP, TEMP, EVAP_P, SNOWPACK3, MELTWATER3, SM3, SUZ3,
                                                                   SLZ3, parBETA, parFC, parK0, parK1, parK2,
                                                                   parLP, parPERC, parUZL, parTT, parCFMAX, parCFR,
                                                                   parCWH, parBETAET)

            SNOWPACK =  torch.clamp(SNOWPACK + (dSNOWPACK1+2*dSNOWPACK2+2*dSNOWPACK3+dSNOWPACK4)/6.0, min=0.0) ;
            MELTWATER = torch.clamp(MELTWATER + (dMELTWATER1+2*dMELTWATER2+2*dMELTWATER3+dMELTWATER4)/6.0 , min=0.0);
            SM = torch.clamp(SM + (dSM1+2*dSM2+2*dSM3+dSM4)/6.0, min=PRECS) ;
            SUZ = torch.clamp(SUZ + (dSUZ1+2*dSUZ2+2*dSUZ3+dSUZ4)/6.0 , min=0.0);
            SLZ = torch.clamp(SLZ + (dSLZ1+2*dSLZ2+2*dSLZ3+dSLZ4)/6.0 , min=0.0);


            Q0_day = parK0 * torch.clamp(SUZ - parUZL, min=0.0)
            Q1_day = parK1 * SUZ
            Q2_day = parK2 * SLZ
            ...
            ETact_day = torch.min(SM, ETact)

            Qsimmu[t, :, :] =  Q0_day + Q1_day + Q2_day
    ...
    def RHS_discrete(self,PRECIP,TEMP,EVAP_P,SNOWPACK0, MELTWATER0, SM0, SUZ0, SLZ0,parBETA, parFC, parK0, parK1, parK2, parLP, parPERC, parUZL, parTT, parCFMAX, parCFR, parCWH, parBETAET):
        PRECS = 1e-5
        SNOWPACK0 = torch.clamp(SNOWPACK0, min=0.0)
        MELTWATER0 = torch.clamp(MELTWATER0, min=0.0)
        SM0 = torch.clamp(SM0 , min=PRECS)
        SUZ0 = torch.clamp(SUZ0, min=0.0)
        SLZ0 = torch.clamp(SLZ0, min=0.0)
        ...
        # Snow process
        SNOWPACK = SNOWPACK0 + SNOW
        melt = parCFMAX * (TEMP - parTT)
        melt = torch.clamp(melt, min=0.0)
        melt = torch.min(melt, SNOWPACK)
        MELTWATER = MELTWATER0 + melt
        SNOWPACK = SNOWPACK - melt
        ...
        # Soil and evaporation
        soil_wetness = (SM0 / parFC) ** parBETA
        soil_wetness = torch.clamp(soil_wetness, min=0.0, max=1.0)
        recharge = (RAIN + tosoil) * soil_wetness

        SM = SM0 + RAIN + tosoil - recharge
        excess = SM - parFC
        excess = torch.clamp(excess, min=0.0)
        SM = SM - excess
        ...
        Q2 = parK2 * SLZ
        SLZ = SLZ - Q2
        return SNOWPACK - SNOWPACK0, MELTWATER-MELTWATER0, SM-SM0, SUZ-SUZ0, SLZ - SLZ0

    def RHS_continuous(self,PRECIP,TEMP,EVAP_P,SNOWPACK, MELTWATER, SM, SUZ, SLZ,parBETA, parFC, parK0, parK1, parK2, parLP, parPERC, parUZL, parTT, parCFMAX, parCFR, parCWH, parBETAET):
        PRECS = 1e-5
        ...
        dSNOWPACK =  (SNOW + refreezing - melt)
        dMELTWATER =  (melt - refreezing - tosoil)
        dSM = (tosoil + RAIN - recharge - excess - ETact)
        dSUZ =  (recharge + excess - PERC - Q0 - Q1)
        dSLZ = (PERC - Q2)
        return dSNOWPACK,dMELTWATER,dSM,dSUZ,dSLZ




class HBVMulTDET_continuous(torch.nn.Module):
    """HBV Model with multiple components and dynamic parameters PyTorch version"""
    # Add an ET shape parameter for the original ET equation; others are the same as HBVMulTD()
    # we suggest you read the class HBVMul() with original static parameters first
    ...
    def __init__(self):
        """Initiate a HBV instance"""
        super(HBVMulTDET_continuous, self).__init__()
    ...
    def forward(self, x, parameters, staind, tdlst, mu, muwts, rtwts,hourstep = 24, bufftime=0, outstate=False, routOpt=False,
        ...
        for t in range(Nstep):
            ...
            for step in range(int(24/hourstep)):
                ...
                SNOWPACK = SNOWPACK+ (SNOW + refreezing - melt)*timestep
                MELTWATER = MELTWATER+ (melt - refreezing - tosoil)*timestep
                SM = SM + (tosoil + RAIN - recharge - excess - ETact)*timestep
                SUZ = SUZ + (recharge + excess - PERC - Q0 - Q1)*timestep
                SLZ = SLZ + (PERC - Q2)*timestep
...
class HBVMulTDET(torch.nn.Module):
    ...
    def forward(self, x, parameters, staind, tdlst, mu, muwts, rtwts,hourstep = 24, bufftime=0, outstate=False, routOpt=False,
        ...
        PRECS = 1e-5  # keep the numerical calculation stable
       # hourstep = 4
        timestep = torch.tensor(hourstep / 24).to(x)
        # Initialization to warm-up states
        if bufftime > 0:
            with torch.no_grad():
                xinit = x[0:bufftime, :, :]
        ...
        for t in range(Nstep):
            ...
            for step in range(int(24/hourstep)):
                ...
                # save components for Q
                Qsimmu0[t, :, :] = Qsimmu0[t, :, :] + Q0
                Qsimmu1[t, :, :] = Qsimmu1[t, :, :] + Q1
                Qsimmu2[t, :, :] = Qsimmu2[t, :, :] + Q2
                ETmu[t, :, :] =ETmu[t, :, :] + ETact
...
class MultiInv_HBVTDModel(torch.nn.Module):
    ...
    def __init__(self, *, ninv, nfea, nmul, hiddeninv, drinv=0.5, inittime=0,hourstep = 24, routOpt=False, comprout=False,
                 compwts=False, staind=-1,scheme = "discrete",RK_scheme = "continuous", tdlst=[], dydrop=0.0, ETMod=False):
        # LSTM Inv + HBV Forward
        ...
        if ETMod is True:
            if scheme == "continuous":
                ...
                self.HBV = HBVMulTDET_continuous()
            elif scheme == "RK":
                if RK_scheme == "continuous":
                    self.HBV = HBVMulTDET_RK(scheme = "continuous")
                elif RK_scheme == "discrete":
                    self.HBV = HBVMulTDET_RK(scheme="discrete")
            else:
                self.HBV = HBVMulTDET()
        else:
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        out = self.HBV(x, parameters=hbvpara, staind=self.staind, tdlst=self.tdlst, mu=self.nmul, muwts=wts, rtwts=routpara,
                          bufftime=self.inittime, routOpt=self.routOpt, hourstep = self.hourstep,comprout=self.comprout, dydrop=self.dydrop)
