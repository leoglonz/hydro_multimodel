...
def fill_Nan_2d(array_2d):
    ...
    for i in range(array_2d.shape[0]):
        ...
        if np.any(nans) and np.sum(non_nans) > 1:
            # Perform linear interpolation using numpy.interp
            if np.sum(nans)>10:
                print("NaN number is larger than 10 in this forcing", np.sum(nans))
            if np.sum(nans)<50:
                array_2d[i, :] = np.interp(x, x[non_nans], slice_1d[non_nans], left=None, right=None)
            else:
                
                array_2d[i, np.where(array_2d[i, :]!=array_2d[i, :])[0]]= np.nanmean(array_2d[i, :])
