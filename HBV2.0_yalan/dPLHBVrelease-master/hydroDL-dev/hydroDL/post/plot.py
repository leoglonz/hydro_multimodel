...
def plotMap(data,
    ...
    if isGrid is True:
        ...
        cs = mm.scatter(
            x, y, c=data, s=5, cmap=cmap, vmin=vmin, vmax=vmax)
    ...
    if pts is not None:
        ...
        for k in range(npt):
            plt.text(
                ...
                fontsize=18)
    if clbar is True:
        color_bar = plt.colorbar(cs, pad=0.05,fraction = 0.11, location='bottom')
    if title is not None:
        ax.set_title(title)
    if ax is None:
        return fig, ax, mm
...
def plotCDF(
    ...
    for k in range(0, len(xLst)):
        ...
        if ref is not None:
            ...
            elif showDiff is 'None':
                legStr = legStr
        ax.plot(xSort, yRank, color=cLst[k], label=legStr, linestyle=linespec[k],lw = linewidth)
    ...
    #plt.arrow(700, 0.45, -200, 0,length_includes_head=True,head_width=0.02, width = 0.01, ec = "r",fc = "r")

    # out_path_ = f'/mnt/sdb/yxs275/check_code/update_plot_results/'
    # plt.savefig(out_path_+'temporal_dmax_cdf.png',bbox_inches='tight',dpi=300)
    # plt.savefig("/data/yxs275/NROdeSolver/output/" + "CDF.png", dpi=300)
    # plt.show(block=True)
