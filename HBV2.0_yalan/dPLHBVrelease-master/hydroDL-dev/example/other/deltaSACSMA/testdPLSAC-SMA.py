...
Nfea = 11 # number of HBV parameters, 13 includes the added one for ET eq
BUFFTIME = 365
routing = True
Nmul = 1
comprout = False
...
tdRep = [] # index of dynamic parameters
...
predtestALL = np.full([len(gageid), len(TtestLst), 2], np.nan)
...
testsave_path = 'CAMELSDemo/dPLSACSMA/' + puN + '/TDTestforc/' + 'TD'+"_".join(tdRepS)+'/' + forType + \
...
for ifold in range(1, len(tarIDLst)+1):
    ...
    filePathLst = master.master.namePred(
          testout, Ttest, 'All_Buff'+str(TestBuff), epoch=testepoch, targLst=['Qr', 'ET'])
