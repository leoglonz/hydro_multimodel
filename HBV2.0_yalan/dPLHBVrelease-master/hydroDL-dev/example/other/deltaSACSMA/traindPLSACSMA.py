...
Nfea = 11 # number of SACSMA parameters. 11:original SACSMA
...
Nmul = 1 # Multi-component model. How many parallel SACSMA components to use. 1 means the original SACSMA.
...
if TDOpt is True:
    ...
    tdRep = [] # When using dynamic parameters, this list defines which parameters to set as dynamic
    ...
    # If 13 not in tdRep list, set below ETMod=False and Nfea=12 to use the original SACSMA without ET shape para
    ETMod = True
...
rootOut = rootDatabase + "output/SACSMA/"
...
# for SACSMA model training inputs
...
xTrain = np.concatenate([forcUN, PETUN], axis=2) # used as SACSMA forcing
...
exp_name = 'CAMELSDemo'
exp_disp = 'dPLSACSMA/' + puN + TDN + forType + '/BuffOpt'+str(buffOpt)+'/RMSE_para'+str(alpha)+'/' + str(randomseed) + \
...
if TDOpt is False:
    ...
    model = rnn.MultiInv_SACSMAModel(ninv=Ninv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
    ...
    optModel = OrderedDict(name='dPLSACSMA', nx=Ninv, nfea=Nfea, nmul=Nmul, hiddenSize=HIDDENSIZE, doReLU=True,
