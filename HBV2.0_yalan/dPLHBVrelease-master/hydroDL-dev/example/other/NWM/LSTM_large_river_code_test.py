...
zarr_save_path = '/projects/mhpi/data/merit_data_in_all_gages/'

dHBV_time_range_all = pd.date_range(start=f'{1980}-01-01', end=f'{2020}-12-31', freq='D')
obs_time_range_all = pd.date_range(start=f'{1980}-01-01', end=f'{2019}-12-31', freq='D')
...
obs_start_id = obs_time_range_all.get_loc(test_span[0])
obs_end_id = obs_time_range_all.get_loc(test_span[-1])+1
...
obs_path = '/projects/mhpi/yxs275/Data/GAGES_2/zarr/'

basin_area_all = np.expand_dims(attributeALL_df["area"].values,axis = 1) 
for gageidx, gagei in enumerate(River_id):
    ...
    obs_i = root_obs['observation']['discharge'][obs_start_id:obs_end_id]
    basin_area = basin_area_all[gageidx][0]
...
filePathLst = [out+"/test_code_Qs.csv"]
...
dataPred = pd.read_csv(  out+"/test_code_Qs.csv", dtype=np.float32, header=None).values
