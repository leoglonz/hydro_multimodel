...
from hydroDL.master import loadModel_multiGPU
...
def forward_on_gpu(item):
    # which GPU to use when having multiple
    gpu_id = item % num_gpus+2
   # torch.cuda.set_device(gpu_id)
    ...
    attributeLst = [ "mean_slope", "mean_elev","mean_aspect","dom_land_cover",
                    "dom_land_cover_frac", "forest_fraction","root_depth_50","root_depth_99","soil_depth_m",
                    "ksat_0_5","ksat_5_15","theta_s_0_5","theta_s_5_15","theta_r_0_5","theta_r_5_15","ksat_0_15_ave",
                 "theta_s_0_15_ave", "theta_r_0_15_ave","ksat_0_5_e","ksat_5_15_e","ksat_0_15_ave_e",
                 "Porosity","Permeability_Permafrost", "MAJ_NDAMS","general_purpose", "max_normal_storage",
    ...
    xTest = xTrain
    testTuple = (xTest[:5,:,:], zTest[:5,:,:])
    testbatch =50 #len(indexes)

    results_savepath = "/data/yxs275/DPL_HBV/HUC10_Output/" + '/dPL_local_daymet_filled_NaN_parallel/'
    ...
    testmodel = loadModel_multiGPU(model_path, device, epoch=testepoch)
    testmodel.inittime = 0

    # model  = testmodel.lstminv.to(device)
    # output = model(torch.tensor(np.swapaxes(zTest[:,:100,:],0,1)).to(device).float())
    print(f"batch {item} is done on GPU {gpu_id}" )
...
# forward_on_gpu(1)
# forward_on_gpu(2)
items = [x for x in np.arange(3)]

pool = multiprocessing.Pool(processes=num_gpus)

b = pool.map(forward_on_gpu, items)
pool.close()
pool.join()
