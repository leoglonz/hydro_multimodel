...
#data_folder = "/storage/home/yxs275/work/DPL_HBV/HUC10/version_1_11_2014_continental_routing/"
...
for item in range(len(iS)):
    ...
    forcing_LSTM_List = ['prcp',  'tmax', 'tmin', ]

    [C, ind1, SubInd] = np.intersect1d(forcing_LSTM_List, forcingBatchLst, return_indices=True)
    forcing_LSTM = forcingBatch[:, :, np.sort(SubInd)]
    ...
    rootOut = "/storage/work/yxs275/DPL_HBV/CONUS_3200_Output/" + '/LSTM_local_daymet_filled_withNaN_NSE/'
    if os.path.exists(rootOut) is False:
        os.mkdir(rootOut)
    out = os.path.join(rootOut, "exp_EPOCH300_BS100_RHO365_HS256_trainBuff365")  # output folder to save results
    ...
    testbatch =100 #len(indexes)

    results_savepath = "/storage/work/yxs275/DPL_HBV/HUC10_Output/" + '/LSTM_local_daymet_filled_withNaN_NSE/'
    if os.path.exists(results_savepath) is False:
        os.mkdir(results_savepath)

    filePathLst = [results_savepath+f"/Qr_{iS[item]}_{iE[item]}"]
