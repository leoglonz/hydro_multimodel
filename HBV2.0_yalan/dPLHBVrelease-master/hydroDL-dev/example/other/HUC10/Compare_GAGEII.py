...
intercepted_folder = "/data/yxs275/CONUS_data/GAGEII_in_HUC10_v2/"
GAGEII_in_HUC10_files = glob.glob(intercepted_folder+"*.npy")
...
for file in GAGEII_in_HUC10_files:
    ...
    gageIDsIn = np.load(file)
    gageI_areaList  = []
    gageIdx_List = []
    IsSmallBasin = False
    for gageI in gageIDsIn:

        # if gageI in smallBasinID:
        IsSmallBasin = True
        gageIdx = np.where(np.array(GAGEII_ID) == gageI)[0][0]
    ...
    if IsSmallBasin:
        ...
        if (gageI_areaList[closest_index]/basinI_area>=0.98 and gageI_areaList[closest_index]/basinI_area<=1.02):
...
np.save(data_folder+"selected_Basin_matched.npy",np.array(selected_Basin))
np.save(data_folder+"selected_GAGE_matched.npy",np.array(selected_GAGE))
