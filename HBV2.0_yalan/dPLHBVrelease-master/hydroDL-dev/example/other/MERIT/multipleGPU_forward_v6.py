...
#Path of model
Model_path =  "/projects/mhpi/yxs275/model/water_loss_model/" + '/dPL_local_daymet_new_attr_water_loss_v6v18_random_batch_filled_data_dynamic_K0_correct_nfea2/'
...
data_folder = '/projects/mhpi/data/conus/zarr/'
...
## Path to save the results
results_savepath = "/projects/mhpi/yxs275/DM_output/" + '/dPL_local_daymet_new_attr_water_loss_v6v18_random_batch_filled_data_dynamic_K0_correct_nfea2/'
...
for file in subzonefile_lst:
    ...
    #if zid not in existing_subzone and zid[:2] == '74':
...
def forward_on_gpu(input):
    ...
    # load attributes
    for aid, attribute_ in enumerate(attributeLst[:-1]) :
    ...
    attribute = np.concatenate((attribute,uparea_zone), axis=-1)
    ...
    attribute_norm_expand = np.expand_dims(attribute_norm, axis=1)
    attribute_norm_expand = np.repeat(attribute_norm_expand, forcing_LSTM_norm.shape[1], axis=1)
    ...
    zTest = np.concatenate([forcing_LSTM_norm, attribute_norm_expand], 2)  # Add attributes to historical forcings as the inversion part
    xTest = xTrain
    ...
    train.testModel_multiGPU(
            testmodel, testTuple,attribute_norm,area_all_merit_info,COMID, region_number,time_range,variables_name,results_savepath, c=None, device =  device, batchSize=testbatch)
