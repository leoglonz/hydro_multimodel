...
save_path_merit_path = '/projects/mhpi/yxs275/Data/merit_data_in_gages_3000_merit/CONUS2800'

with open('/projects/mhpi/yxs275/Data/merit_data_in_gages_3000_merit/' + 'merit_idx.json') as f:
...
Ninv = forcing_norm2.shape[-1]+attribute_norm2.shape[-1]
...
Nmul = 4 # Multi-component model. How many parallel HBV components to use. 1 means the original HBV.
drinv = 0.5
...
model_name= 'waterlossv11'   ###Which water loss formula to use
model = rnn.MultiInv_HBVTDModel_water_loss_v7(ninv = Ninv, nfea = Nfea, nmul = Nmul, hiddeninv = HIDDENSIZE,drinv = drinv, inittime=BUFFTIME,
...
# rootOut_pre = "/projects/mhpi/yxs275/model/water_loss_model/"+'/dPL_local_daymet_new_attr_water_loss_v4/exp_EPOCH50_BS100_RHO365_HS1512_HS232_trainBuff365/'

# testepoch = 62

# print("Load model from ", rootOut_pre)
# old_model = loadModel(rootOut_pre, epoch=testepoch)
...
rootOut = "/projects/mhpi/yxs275/model/water_loss_model/"+'/dPL_local_daymet_new_attr_water_loss_v6v10_random_batch'
if os.path.exists(rootOut) is False:
    os.mkdir(rootOut)
out = os.path.join(rootOut, f"exp_EPOCH{EPOCH}_BS{BATCH_SIZE}_RHO{RHO}_HS{HIDDENSIZE}_MUL{Nmul}_trainBuff{BUFFTIME}_test") # output folder to save results
...
def main():
    trainedModel = train.trainModel_multiGPU(
        model,
        forcTuple,
        streamflow_trans,
        None,
