...
streamflow = np.load(data_folder+"water_loss_data/runoff_global_3600.npy")[:,train_start_idx:train_end_idx]
...
xTrain2 = np.full((len(Merit_all),len(trainingtime),len(forcing_list_water_loss)),np.nan)
attr2 = np.full((len(Merit_all),len(attributeLst_water_loss)),np.nan)


merit_time = pd.date_range('1980-01-01',f'2020-12-31', freq='d')
merit_start_idx = merit_time.get_loc(trainingtime[0])
merit_end_idx = merit_time.get_loc(trainingtime[-1])+1
...
drinv2 = 0.5
model_name= 'waterlossv14'   ###Which water loss formula to use
...
lossFun = crit.NSELossBatch(np.nanstd(streamflow, axis=1))
