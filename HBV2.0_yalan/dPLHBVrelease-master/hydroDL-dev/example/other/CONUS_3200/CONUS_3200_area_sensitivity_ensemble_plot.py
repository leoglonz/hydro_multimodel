...
ensemble_path = '/projects/mhpi/yxs275/model/scale_analysis/'
mode  = '/scaling_analysis'
filePathLst = [ensemble_path +mode+ f"/allBasinParaHBV_area_new_base", ensemble_path +mode+ f"/allBasinParaRout_area_new_base",ensemble_path+mode+f"/allBasinFluxes_area_new_base"]
...
ensemble_dataPred = allBasinFluxes_base[:,-len(test_span):,0]

evaDict = [stat.statError(ensemble_dataPred, test_obs)]
evaDictLst = evaDict
...
NSE_ensemble_loss_model = dataBox[0][0]
...
# pred_df = pd.DataFrame(ensemble_dataPred.transpose(), index=test_span)

# monthly_means_pred = pred_df.resample('M').mean()

# obs_df = pd.DataFrame(test_obs.transpose(), index=test_span)
...
#         temp.append(data)
#     dataBox.append(temp)
# print("monthly HBV model 'NSE', 'KGE','FLV','FHV', 'lowRMSE', 'highRMSE' ,'rdMax','absFLV','absFHV'",
#       np.nanmedian(dataBox[0][0]),
#       np.nanmedian(dataBox[1][0]), np.nanmedian(dataBox[2][0]), np.nanmedian(dataBox[3][0]),
#       np.nanmedian(dataBox[4][0]), np.nanmedian(dataBox[5][0]), np.nanmedian(dataBox[6][0]), np.nanmedian(dataBox[7][0]), np.nanmedian(dataBox[8][0]))
...
ensemble_bias_pred,_= bias_meanflowratio_calc(ensemble_dataPred, test_obs)
...
prediction_out = ensemble_path+mode
...
# plt.title("NSE loss model")
plt.tight_layout()
plt.savefig(ensemble_path+"Partial_dependence_variables_S.png", dpi=100)
...
delta_Q7_area_precp = np.load(ensemble_path+ '/delta_Q7_area_precp.npy')
delta_Q7_area_T = np.load(ensemble_path+'/delta_Q7_area_T.npy')
peak_delta_flow_area_precp= np.load(ensemble_path+'/peak_delta_flow_area_precp.npy')
peak_delta_flow_area_T = np.load(ensemble_path+'/peak_delta_flow_area_T.npy', allow_pickle=True)
...
plt.tight_layout()
plt.savefig(ensemble_path+"Partial_dependence_variables_forcing.png", dpi=300)
plt.show(block =True)
print("Done")
...
plt.tight_layout()
plt.savefig(ensemble_path+"Partial_dependence_variables_forcing_S.png", dpi=300)
plt.show(block =True)
print("Done")
...
ylabels = ['Scale-dependence ratio of\nwater supply  $R$($Q$/$Q_{minA}}$)', 'Scale-dependence ratio of\ngroundwater contribution $R$($Q_{base}$/$Q$)','Scale-dependence ratio of\npeak flow sensitivity $R$($\Delta Q_{max}/Q_{max}$)\nresponse to +1% precipitation','Scale-dependence ratio of\nlow flow sensitivity $R$($\Delta Q_{7}/Q_{7}$)\nresponse to +1$^{\circ}$ temperature ']
...
plt.tight_layout()
plt.savefig(ensemble_path+"Variable_change_map.png", dpi=300)
print("Done")
...
plt.tight_layout()
plt.savefig(ensemble_path+"Variable_change_map_S.png", dpi=300)
print("Done")
...
plt.tight_layout()
plt.savefig(ensemble_path+"aridity_map.png", dpi=300)
print("Done")
...
plt.savefig(ensemble_path+"Partial_dependence_parameters.png", dpi=300)
print("Done")
...
#bins = np.linspace(lower_bound, upper_bound, nbin + 1)
bin_length = (upper_bound - lower_bound) / (nbin-1)
bins =np.array([0,8000,16000,24000,32000,50000])
bins_split =np.array([0,1000,2000,5000,10000,50000])
# bins_split =np.array([0,0.75,1.2,2,7])
...
plot1 = ax.boxplot( [ ensemble_bias_pred[np.where(area_bin_index == i)][~np.isnan(ensemble_bias_pred[np.where(area_bin_index == i)])] for i in range(1,nbin+1) ], vert=True,showfliers=False, positions=bins[:-1]+2*bin_length/4.0,patch_artist=True,boxprops=dict(facecolor="pink", color="k") ,widths = bin_length/6)
...
for i in range(1,nbin+1):
    ...
    num = len(area_selected[np.where(area_bin_index == i)])
    ax.text(bin_length/6.0+(i-1)*bin_length+lower_bound,y_upper-0.08*(y_upper-y_lower), f'{num} sites')


ax.set_ylabel("Relative bias ratio")
# ax.set_xlabel(r"Aridity")
ax.set_xlabel(r"Catchment area (km$^2$)")
