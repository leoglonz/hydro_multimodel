...
warmup_span = pd.date_range('1980-10-01',f'1995-10-01', freq='d', closed='left')
...
forcingAll = np.load("/storage/work/yxs275/DPL_HBV/CONUS_3200_data/allBasin_localDaymet.npy")[:,index_start:index_end+1,:]
...
attributeGAGEII  = np.load(data_folder+"test_attr.npy")
...
attribute_file = '/storage/work/yxs275/DPL_HBV/CONUS_3200_data/attributes.csv'
...
# streamflow_trans = streamflow_trans[selected_gageIdx,:,:]
# xTrain = xTrain[selected_gageIdx,:,:]
# attribute = attribute[selected_gageIdx,:]
...
rootOut = "/storage/work/yxs275/DPL_HBV/CONUS_3200_Output/" + '/dPL_local_daymet_filled_NaN/'
if os.path.exists(rootOut) is False:
    os.mkdir(rootOut)
out = os.path.join(rootOut, "exp_EPOCH50_BS100_RHO365_HS512_trainBuff365_fold1")  # output folder to save results
...
train.testModel(
    testmodel, testTuple_fold, c=None, batchSize=testbatch, filePathLst=filePathLst)
...
evaDict = [stat.statError(dataPred[:,warmup_for_testing:,0], streamflow_trans_fold[:,warmup_for_testing:,0])]
evaDictLst = evaDict
