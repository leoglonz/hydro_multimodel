...
x = np.load("/data/yxs275/NROdeSolver/CAMELSData/HBV_test.npy")[:,4500:,:]
y = np.load("/data/yxs275/NROdeSolver/CAMELSData/obs_test.npy")

z = np.load("/data/yxs275/NROdeSolver/CAMELSData/LSTM_test.npy")[:,4500:,:]
...
model = rnn.MultiInv_HBVTDModel(ninv=Ninv, nfea=Nfea, nmul=Nmul, hiddeninv=HIDDENSIZE, inittime=BUFFTIME,
                                routOpt=routing, comprout=comprout,scheme = "RK",RK_scheme = "continuous", hourstep = 4,compwts=compwts, staind=staind, tdlst=tdRep,
                                dydrop=dydrop, ETMod=ETMod)
lossFun = crit.RmseLossComb(alpha=alpha)

#forcTuple = [data,forcing_norm]

rootOut = "/data/yxs275/DPL_HBV/output/"+'/record_16_4hour_RKcontinuous_corrected/'
...
evaDict = [stat.statError(dataPred[:,-y.shape[1]:,0], y[:,:,0])]
evaDictLst = evaDict
...
print("LSTM model'NSE', 'KGE','absFLV','absFHV', 'lowRMSE', 'highRMSE' ,'rdMax','absFLV','absFHV'",
